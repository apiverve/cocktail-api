...
class CocktailValidationException implements Exception {
  final List<String> errors;

  CocktailValidationException(this.errors);

  @override
  String toString() => 'CocktailValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://apiverve.com/marketplace/cocktail?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [name] - The name of the cocktail for which you want to get the recipe (e.g., martini)
class CocktailClient {
  ...
  CocktailClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/cocktail',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CocktailValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CocktailValidationException(errors);
    }
  }

  /// Execute a request to the Cocktail Recipe API.
  ///
  /// Parameters are validated before sending the request.
  Future<CocktailResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CocktailException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CocktailResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CocktailException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CocktailException('Resource not found');
      } else {
        throw CocktailException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CocktailException) rethrow;
      throw CocktailException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cocktail Recipe API client.
class CocktailException implements Exception {
  final String message;

  CocktailException(this.message);

  @override
  String toString() => 'CocktailException: $message';
}
